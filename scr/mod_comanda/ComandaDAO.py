...
def get_comanda(id_comanda: int, current_user:Annotated[User, Depends(get_current_active_user)]):
    try:
        session = db.Session()
...
def get_comanda(status: int, current_user:Annotated[User, Depends(get_current_active_user)]):
    try:
        session = db.Session()
        # busca filtrando pelo status
        aux_dados = session.query(ComandaDB, FuncionarioDB, ClienteDB)\
...
def post_comanda(corpo: Comanda, current_user:Annotated[User, Depends(get_current_active_user)]):
    try:
        session = db.Session()
...
def put_comanda(corpo: Comanda, current_user:Annotated[User, Depends(get_current_active_user)]):
    try:
        session = db.Session()
...
def post_comanda_item(corpo: ComandaProdutos, current_user:Annotated[User, Depends(get_current_active_user)]):
    try:
        session = db.Session()
...
def get_comanda_item(comanda_id: int, current_user:Annotated[User, Depends(get_current_active_user)]):
    try:
        session = db.Session()
...
def put_comanda_item(corpo: ComandaProdutos, current_user:Annotated[User, Depends(get_current_active_user)]):
    try:
        session = db.Session()
...
def get_comanda_total(comanda_id: int, current_user:Annotated[User, Depends(get_current_active_user)]):
    try:
        session = db.Session()
